# -*- coding: utf-8 -*-
"""
Created on Sun Apr  3 13:26:58 2022
...
"""
es main hum differnt model banaty hain or check krty hain k konsa model acha predict krta ha
model name Random Forest, XGBoost and LGBM

random forest or XGboost dono ka tareeka B same ha or processing speed B same ha or accuracy B takreeban same ha

lakin LGBM k tareeka thora different ha or processing speed dono sy bht zada fast ha

LGBM ki sari detail neechay ha us model k samny line by line detail
...
d_train = lgb.Dataset(X_train, label=y_train)         #es main hum Dataset a function use kiya hum ny test or train data ko 1 data set k andr store krny k liye

lgbm_params = {'learning_rate':0.05, 'boosting_type':'dart',    #Try dart for better      #es main hum ny differnet functions diye jis ny hamara model fast hota 
...
#model  ko train krty waqt jo paramenter hum ny banay thy jis sy hamara model fast ha wo or data set jis main test or train data ha us pr apply kr k apny model ko train kiya
...
prediction_lgb = np.array([np.argmax(i) for i in prediction_lgb])     #Q k hamara function hamain yield values deta ha jo on the fly values ko generate krta ha jis sy classification nai ho sakt es liye hamain list main store krna prt ha classification k liye ta k hum predict kr sakai apni values koo
